...
function CID_get_flag($ip) {
	...
	$output = str_replace("%IMAGE_BASE%", get_stylesheet_directory_uri().'/ip/flags', $output);
...
function CID_get_flag_without_template($ip, $show_image = true, $show_text = true, $before = '', $after = '') {
	...
	if ($show_image)
		$output = '<img src="'.get_stylesheet_directory_uri().'/ip/flags/' . $code . '.png" title="' . $name . '" alt="' . $name . '" class="country-flag" width="18" height="14" />';
	if ($show_text)
		$output .= ' ' . $name;
...
/* 浏览器 */
$CID_image_url = get_stylesheet_directory_uri()."/ip/browsers/";
...
function CID_friendly_string($browser_name = '', $browser_code = '', $browser_ver = '', $os_name = '', $os_code = '', $os_ver = '', $pda_name= '', $pda_code = '', $pda_ver = '') {
	...
	$output = stripslashes('<span class="WB-OS"><img src="%IMAGE_BASE%/%BROWSER_CODE%.png" title="%BROWSER_NAME%" alt="%BROWSER_NAME%" height="16" width="16" /> <img src="%IMAGE_BASE%/%OS_CODE%.png" title="%OS_NAME% %OS_VERSION%" alt="%OS_NAME% %OS_VERSION%" height="16" width="16" /> </span>');
	...
	$output = str_replace("%IMAGE_BASE%", get_stylesheet_directory_uri()."/ip/browsers", $output);
