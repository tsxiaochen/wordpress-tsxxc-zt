...
{
    global $post;
   if ( is_array( $boxes ) )  foreach ($boxes as $meta_box) {
...
function ts_seo_post_metas()
{
    global $seo_metas;
    post_meta_set($seo_metas);
}

function ts_seo_post_metas_save($post_id)
{
    global $seo_metas;
    save_post_meta_data($post_id, $seo_metas);
}

function ts_seo_post_meta_box()
{
    if (function_exists('add_meta_box')) {
        add_meta_box('ts_seo_post_meta_box', 'SEO设置', 'ts_seo_post_metas', 'post', 'normal', 'high');
    }
}

add_action('admin_menu', 'ts_seo_post_meta_box');
add_action('save_post', 'ts_seo_post_metas_save');
...
function ts_basic_post_metas()
{
    global $basic_metas;
    post_meta_set($basic_metas);
}

function ts_basic_post_metas_save($post_id)
{
    global $basic_metas;
    save_post_meta_data($post_id, $basic_metas);
}

function ts_basic_post_meta_box()
{
    if (function_exists('add_meta_box')) {
        add_meta_box('ts_basic_post_meta_box', '基本设置', 'ts_basic_post_metas', 'post', 'normal', 'high');
    }
}

add_action('admin_menu', 'ts_basic_post_meta_box');
add_action('save_post', 'ts_basic_post_metas_save');